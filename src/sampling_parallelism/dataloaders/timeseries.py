...
class TimeseriesDatasetUnsqueeze(Dataset):
    ...
    def __getitem__(self, index):
        history = torch.tensor(self.raw[index:index+self.input_length], dtype=torch.float32)
        prediction = torch.tensor(self.raw[index+self.input_length:index+self.input_length+self.output_length], dtype=torch.float32)
